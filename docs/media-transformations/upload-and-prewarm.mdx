...
| `url` | string | Base transformation URL |
| `prewarmedUrls` | string[] | Cached variants (images) |
| `prewarmErrors` | object[] | Prewarm failures per variant (images) |
| `queuedTransformationUrls` | string[] | Queued variants (videos) |
| `queueErrors` | object[] | Queue failures per variant (videos) |

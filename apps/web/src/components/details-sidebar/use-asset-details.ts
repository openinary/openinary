...
export function useAssetDetails(assetId: string | null) {
  ...
  const asset = useMemo(() => {
    if (!assetId || !treeData) return null;

    return findAssetInTree(treeData, assetId);
  }, [assetId, treeData]);
  ...
  const fetchFileMetadata = async (path: string) => {
    try {
      ...
      const response = await fetch(
        `${apiBaseUrl}/storage/${encodedPath}/metadata`,
        {
          method: "GET",
          credentials: "include",
        },
  ...
  const fetchOptimizedSize = async (path: string) => {
    try {
      ...
      if (headResponse.ok) {
        ...
        const isOriginal =
          headResponse.headers.get("X-Original-Video") === "true";
        const optimizedSizeHeader =
          headResponse.headers.get("X-Optimized-Size");
  ...
  const { status: videoStatus, progress: videoProgress } = useVideoStatus(
    videoPath,
    !!asset,
  );
...
export function useCurrentAssetDetails(onOpenChange?: (open: boolean) => void) {
  ...
  }, [assetDetails.asset, onOpenChange]);

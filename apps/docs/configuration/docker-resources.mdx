...
Matches the commented block in `docker.env.example`. The CPU limit is what caps [video worker auto-detection](/configuration/video-processing#video-max-concurrent), so raise it alongside memory or the extra RAM buys you no extra workers.

...
You end up with a `<FileUploader />` in your React app, a route on your own API that mints a short-lived upload signature, and an API key that never reaches the bundle. Two shadcn commands install both pieces as source you own. This is the setup for a React frontend with a separate backend, a Vite app with an Express, Hono or Fastify API behind it. The flow is identical on Openinary Cloud and on a self-hosted instance, and the few steps that differ say which is which.
...
Every snippet below is complete. Copy each file into your project as you go.

<Steps>
  <Step title="Register the Openinary registry">
    The uploader installs as source you own, not as a package. Point shadcn at the namespace once. You need a project already set up for shadcn/ui (`components.json` present) and the shadcn CLI **v3 or newer**, which is what resolves namespaced registries.
  ...
  <Step title="Add the component and the signing helper">
    ...
    **Run that second command in the backend project, or move the file there.** `upload-token` reads an API key, and anything the Vite app imports ends up in the bundle.

    No shadcn in this project? Don't run `shadcn init` for it, that would restructure a project that never asked for it. Take `use-file-upload.ts` alone, verbatim, from [the registry JSON](https://openinary.dev/r/file-uploader.json), which carries each file's target and full content. The hook holds all the logic and no styling, so you write the markup your codebase would have written anyway.
  </Step>
...
<Warning>
  **Derive `folder` server-side, from the authenticated user.** A folder read from the request body is a folder any visitor can choose, including someone else's. Authenticate the route too, with whatever this app already uses: it hands out an upload grant. `signUpload()` does neither for you.
</Warning>

The route belongs in the API, not in the app that renders the uploader. A frontend on `app.example.com` calling an API on `api.example.com` needs CORS on **your own** signing route, which is separate from anything Openinary is configured with.
...
Treat the original as a confirmation, not a gallery. It is full-size bytes and one CDN request, which is right for the single file that just arrived and wrong for a grid of fifty, use transformations there. Only jpg, png, webp, avif and gif decode in a browser, anything else is a link. Drop the component's local preview once the upload finishes rather than showing both.
...
<AccordionGroup>
  ...
  <Accordion title="401 on POST /upload after signing">
    The signature's `expires` passed before the upload started, or the folder used to sign doesn't match the one submitted. The component re-signs on retry, so this normally means the signing route and the instance disagree. On self-hosted, also check for clock skew and for an `API_SECRET` that changed after the signature was minted.
  </Accordion>

...
<Note>
  On Openinary Cloud the same component works with a different configuration, see [File Uploader on Cloud](/cloud/file-uploader).
</Note>
...
<ParamField path="multiple" default="true" type="boolean">
  Allow selecting more than one file.
</ParamField>

<ParamField path="disabled" default="false" type="boolean">
  Blocks file selection and drag and drop. Use it while a parent form is
  submitting, or before the user is allowed to upload.
</ParamField>
...
<ParamField path="maxSize" default="52428800" type="number">
...
<ParamField path="maxFiles" type="number">
  ...
  /upload/sign` does not encode a file-count limit, so it isn't enforced server-side.
</ParamField>

<ParamField path="transformations" type="string[]">
  Transformation segments to pre-warm after upload, e.g. `["w_800,f_webp"]`. See [Upload & Pre-warm](/media-transformations/upload-and-prewarm).
</ParamField>

<ParamField path="autoUpload" default="false" type="boolean">

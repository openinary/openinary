...
<ParamField path="signature" type="string" required>
  16-character HMAC-SHA256 signature, truncated from the full digest. Computed server-side when generating the signed URL using your `API_SECRET`.

`s--a1b2c3d4e5f6a7b8` (16 hex chars)

The server rejects any other length with `400` before it even checks the value.

...
| `east`   | Right edge                                       |
| `west`   | Left edge                                        |
| `face`   | Saliency crop: keeps the region Sharp scores as most interesting. Usually the face in a portrait, but it is not a face detector and has no notion of faces. |
...
<ParamField path="b" type="string">
  Background color for `c_pad` (padding fill), `a_<angle>` (rotation fill), and `r_<radius>` (corner fill). Accepts `rgb:` plus a 6-digit hex value, a bare `#hex`, or one of the keywords `transparent`, `white` and `black`. Also aliased as `bg`.

`b_rgb:ffffff` (white) · `b_rgb:f5f5f5` (light gray) · `b_transparent` · `b_black`
...
<a id="q-image"></a>
<ParamField path="q" type="integer" default="auto">
  Compression quality from 1 (smallest file) to 100 (highest quality), or `auto`.

  Left unset, quality is chosen per output format rather than from one fixed
  number: 80 for AVIF, 85 for WebP, 90 for JPEG, and 100 for PNG since it is
  lossless. Large files shift that by a few points, and the result is clamped to
  the 70 to 95 range. An explicit value between 1 and 100 is used as given, with
  no clamping.
...
| `gif`          | Animated or legacy assets                                                    |
| `auto`         | Encode every supported candidate, compare byte counts, serve the smallest    |

</ParamField>

### Parameter aliases

The URL parser accepts more spellings than the tables above list. They are here so a URL written against Cloudinary's vocabulary still parses.

| Parameter | Also accepts                                                                                     |
| --------- | ------------------------------------------------------------------------------------------------ |
| `c`       | `lfill`, `fill_pad`, `limit`, `mfit`, `thumb`, `lpad`                                            |
| `g`       | `c`, `n`, `s`, `e`, `w`, `faces`, `face_center`, `faces_center`, `north_center`, `south_center`   |
| `b`       | `bg`                                                                                              |

<Warning>
  Only the short key works. The parser splits a segment at its first underscore
  and matches the result against a fixed key list, so a long-form name such as
  `thumbnail_time_5` is read as the key `thumbnail`, fails to match, and the
  whole segment is then treated as a folder name, which ends in a `404`.
</Warning>

<Note>
  Video URLs silently ignore the image-only parameters `a`, `ar`, `r` and `b`.
  They parse, so the URL still works, but the output is re-encoded without them
  applied.
</Note>
...
Square thumbnail cropped around the most visually salient region, which on a portrait is normally the face.
...
`tt_5.5`
  ...
  <Warning>
    Only the short key works in a URL. The URL parser splits a segment at its
    first underscore and matches the result against a fixed key list, so
    `thumbnail_time_5` parses as the key `thumbnail`, fails, and is then treated
    as a folder name, which ends in a `404`.
  </Warning>

</ParamField>

<a id="q-video"></a>
<ParamField path="q" type="integer | auto" default="60">
  ...
  CRF is derived as `round(51 - q / 100 * 33)`.
...
| 10        | 48  | Low quality, minimum size     |

  The default favors encoding speed over size, which matters most on large
  source videos. Raise `q` when output quality matters more than how long the
  job takes.
...
**Output:** MP4 (H.264) by default. `f_webm` and `f_mov` are also accepted.

---

## Pass-through formats

Audio and 3D files are stored and delivered, but never transformed. Request them through `/t/` with no transformation segment and the stored bytes are streamed back untouched, with the right `Content-Type`.
...
```http
GET /t/audio/podcast.mp3
GET /t/models/duck.glb
```

<Warning>
  Adding transformation parameters to one of these returns `400`, not the
  original:

```
GET /t/w_500/audio/podcast.mp3
→ 400  audio/podcast.mp3 can't be transformed. Request /t/audio/podcast.mp3 for the original.
```

  This is deliberate. Silently ignoring the parameters would cache a "resized"
  variant that is really the untouched file.
</Warning>

## Input validation

Uploads are validated by content, not by the extension or the declared MIME type: the first bytes of the body must match the format the file claims to be. A `.jpg` holding an HTML document is rejected at upload rather than stored.

Because delivery serves stored bytes as they are, this is what keeps a stored file from being served as something it is not. Responses also carry `X-Content-Type-Options: nosniff`, and the content type comes from the shared media table rather than from sniffing the body.

HEIC and HEIF uploads are the one exception to bytes-in-bytes-out: they are converted to JPEG at upload time and stored with a `.jpg` path, since browsers cannot display HEIC. PSD files are accepted as a transformable input, and always come out as one of the standard image formats.
...
**Status codes:** `200` success · `202` video transformation still processing · `400` transformation parameters on a non-transformable type · `404` file not found · `500` processing error

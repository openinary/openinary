...
<Note>
  A folder path returns `400` on local disk storage but `404` on a cloud
  storage backend, which has no directory to stat. Treat both as "not a file"
  rather than branching on the code.
</Note>

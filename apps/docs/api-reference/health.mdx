...
<ResponseField name="database.permissions" type="string">
  Octal file permissions, three digits with no leading zero (for example `600` or `644`).
</ResponseField>
...
<ResponseField name="database.warning" type="string">
  Present only when the database file is larger than 1 GB. Permission problems are reported in `security_warning` instead.
</ResponseField>

...
`core` and `ui` are workspace packages rather than npm releases: the apps
consume them through `workspace:*`, so nothing has to be published between
fixing one and shipping it. To use them in your own app, go through the
[shadcn registry](/guides/file-uploader).

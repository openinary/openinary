...
async function fetchEnvTemplateAtTag(tag: string): Promise<string | null> {
  try {
    const res = await fetch(
      `https://raw.githubusercontent.com/${REPO}/${tag}/docker.env.example`,
      {
...
async function syncEnvVars(project: Project, targetTag: string): Promise<void> {
  ...
  const currentContent = (await fs.pathExists(envPath))
  ...
  hint(
    `Added new environment variable${missingKeys.length > 1 ? "s" : ""}: ${missingKeys.join(", ")}`,
  );
}
...
export async function runUpgrade(options: UpgradeOptions): Promise<void> {
  ...
  const changelogText = changelogRaw
    ? renderChangelog(
        sectionsBetween(changelogRaw, isPinned ? current : "0.0.0", target),
      )
    : null;

  if (options.dryRun) {
    note(
      `${isPinned ? current : "unpinned (latest)"} → ${target}`,
      "Upgrade plan",
    );
  ...
  await withSpinner("Syncing environment variables", () =>
    syncEnvVars(project, target),
  );
  ...
  await fs.writeFile(
    envPath,
    serializeEnv(upsertVar(envLines, "IMAGE_TAG", target)),
  );

  await writeProjectConfig(project.dir, { ...project.config, version: target });

  await withSpinner(`Downloading ${target}`, () =>
    compose(project.dir, ["--profile", project.config.mode, "pull"]),
  );
...
export function buildUpgradeCommand(): Command {
  return new Command("upgrade")
    .description(
      "Update this project to the latest compatible Openinary version",
    )
    .option(
      "--to <version>",
      "Upgrade to a specific version instead of the latest",
    )
